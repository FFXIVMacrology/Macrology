...
部分文本指令在主命令之外，还需要加一些具体的可变参数，这些参数被称为子命令。子命令通过空格和文本指令的其他部分分割，从文本指令的第一个空格起开始编号。
...
### 宏面板
按ESC，选择用户宏，或者选择右下角快捷菜单的系统分类内的用户宏来打开你的用户宏面板 (你也可以在键位设置中设置一个快捷键)
...
- 技能名必须为原始技能名，不能是衍生技能名，例如`纷乱雪月花`必须写作`居合术`，`赤神圣`必须写作`赤疾风`。<!-- TODO: 可选式衍生技能? -->
...
因为这样的缺点，这个指令我们一般不会单独用于GCD技能，往往用于能力技，以及生产采集职业。
...
- 使用 `set` 时，必须指定原始技能名，衍生技能名不能正常摆放。例如：纷乱雪月花应写作居合术。 <!-- TODO: 可选式衍生技能? -->
...
- 使用 `item` 时，只能指定存在于背包中的道具。（如果是HQ道具，则必须粘贴上HQ符号）
...
例如，黑魔的星极火和灵极冰状态是系统规定的，在星极火状态下，冰澈被系统强制禁用，这就是一个技能状态。

再例如，武士打出阵风后，一般情况下不会紧跟着打雪风、士风、花车，这是人为规定的状态，也是一个技能状态。

为了防止在一个技能状态中失误使用错误的技能，并节约键位，更容易按到正确的技能，我们可以通过某种方法，把不应该使用的技能从热键栏上消去，把应该使用的技能放到最顺手的键位上，高效地利用顺手的键位，这样的体系称为状态机。而翻页宏便是实现这种状态机的优秀选择。

所谓翻页宏，早期是利用`/hotbar change`指令切换1号热键栏的显示进行翻页，后来升级为利用`/hotbar copy`指令使任意热键栏复制指定的热键栏内容实现翻页。

在战斗中，我们可以将各技能状态分别保存在一条热键栏上，多条热键栏通过翻页轮流利用热键栏上的顺手键位，可以让技能顺手、节省键位和防止误触。

### 连击翻页宏
技能状态的改变主要来自技能释放，所以将释放某改变技能状态的技能与翻页绑定是不增加操作量的最好选择。

随之而来的问题是，如何让这个改变技能状态的技能绑定在宏里后仍然进入技能队列？只有普通的技能热键可以进入技能队列，要实现这一点，则必须设法点击普通技能热键。

在翻页时，热键栏会在按下宏的瞬间切换，因此，我们可以在翻页后同一个位置放上普通技能热键，这样只要按键连打就可以自然的点击普通技能热键，就像这样：
...
无论当前状态，我们要开始一次新的单体连击时，总要先打精准刺，然后在贯通刺和开膛枪之中选择一个，我们将其放在一起，这就是10号热键栏上所显示的状态。

假设接下来我们打贯通刺，那么再下一个技能就应该是苍穹刺，而不可能再是开膛枪。我们将苍穹刺替换掉开膛枪，使其放在一起，这就是4号热键栏上所显示的状态。链接这两个状态的关键是使用贯通刺，因此我们设计贯通刺宏如下：
...
接下来，我们打出苍穹刺，然后再下一个技能就应该是龙牙龙爪，而不可能再打贯通刺了。我们将龙牙龙爪替换掉贯通刺，使其放在一起，这就是5号热键栏上所显示的状态。链接这两个状态的关键是使用苍穹刺，因为我们设计苍穹刺宏如下：
...
对比一下上面两个自然段便能发现，两段描述几乎完全一样，我相信剩余的不必再赘述。在这张图中，每个纵列的相同技能图标中，只有一个不带有右上角的小齿轮的真实热键，剩余的带有小齿轮的宏，则是使用`/hotbat copy`将含有对应真实热键的热键栏复制到1号热键栏上。

图中状态4是使用贯通刺后、状态5是使用苍穹刺后、状态6是使用龙牙龙爪/龙尾大回旋后、状态7是使用樱花缭乱后、状态8是使用开膛枪后、状态9是AOE连击中、状态10是使用精准刺后。正常完成龙骑士单体循环10连的技能状态切换顺序是：`10 -> 8 -> 7 -> 6 -> 10 -> 4 -> 5 -> 6`，按键顺序是`13232 12323`。该状态机只有2和3键在不断变化，1和4键作为连击的起点始终保持不变，可以在任何时候从头开始单体或AOE连击。

这个例子只显示了热键栏上的前4格。如果该热键栏上还有其他热键，应将其在每一个技能状态中都复制出来。

需要注意的是，在这样的状态机下，热键栏1成为了一个显示器，它接受从其他热键栏复制来的热键，而自身不储存任何热键。在热键栏1上进行的修改，会在下一次翻页时被清除掉，要永久更改其内容需要在实际储存技能状态的热键栏更改。

使用较旧的`/hotbar change`式翻页宏可以避免这一问题，但因为该指令只能使用最多10条热键栏，而不像`/hotbar copy`式翻页宏可以使用430条热键栏，所以一般不推荐。
...
### 单体群体翻页宏
从6.0开始，很多职业的群体攻击都变成了与单体攻击逻辑完全相同的换皮。蝰蛇剑士有5个单体和5个群体键位一一对应。
<!-- 修改区 -->
